...
# ALL THIS IS DONE TO FIND THE CENTER OF THE PUPIL
...
def get_center_map(eye_img_one_channel):
    # GRADIANT (DERIVATIVE) -> SHOWS THE MAXIMUM CHANGES ALONG A DIRECTION
    grad_x_img = cv2.Sobel(eye_img_one_channel, ddepth=cv2.CV_32F, dx=1, dy=0, ksize=5)
    grad_y_img = cv2.Sobel(eye_img_one_channel, ddepth=cv2.CV_32F, dx=0, dy=1, ksize=5)
    ...
    eye_img_inv = 255 - eye_img_one_channel
    darkness_weights = eye_img_inv / __inv_intensity_weight_divisor

    accumulator = np.zeros(eye_img_one_channel.shape[:2], dtype=np.float32)
    indicies_grid = np.indices(accumulator.shape[:2])
    indicies_shape = indicies_grid.shape[1:3]

    for y in range(eye_img_one_channel.shape[0]):
        for x in range(eye_img_one_channel.shape[1]):
    ...
    num_gradients = eye_img_one_channel.shape[0] * eye_img_one_channel.shape[1]
    return accumulator / num_gradients


# FINDS THE CENTER OF THE PUPIL USING THE IMAGE GRADIENTS -> I CANT REPLACE THIS WITH IRIS DETECTION MODEL
def find_pupil(eye_img_bgr, debug_index=False):
    ...
    if debug_index:
        ...
        cmap_norm = cv2.normalize(center_map, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        ...
        if debug_index == 2:
            full_debug_img = image_utils.stack_imgs_vertical([debug_img,eye_img_r_debug, center_map_big])
...
if __name__ == '__main__':
    ...
    tic = time()
    for a in range(100):
        pupil_x0, pupil_y0 = find_pupil(eye_img, debug_index=2)
