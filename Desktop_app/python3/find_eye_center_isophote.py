...
def get_center_map(eye_img):
    ...
    weight_edge = cv2.normalize(curvedness, 0, 255 * __weight_ratio_edge,
                                norm_type=cv2.NORM_MINMAX)  # higher weight to stronger edges
    weight_middle = cv2.normalize((255 - eye_img), 0, 255 * __weight_ratio_darkness,
...
def find_pupil(eye_img_bgr, debug_index=False):
    ...
    if debug_index:
        ...
        if debug_index == 2:
            full_debug_img = image_utils.stack_imgs_vertical([debug_img, overlay_img])
