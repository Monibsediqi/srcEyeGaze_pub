...
from ransac_eye_lids import ransac_line, ransac_parabola
...
def find_upper_eyelid(eye_img, debug_index):
    ...
    # Mask with circles
    cv2.circle(filter_img, (int(3 * filter_img.shape[1] / 7), int(filter_img.shape[0] / 2)), int(filter_img.shape[1] / 4), 0, -1)
    cv2.circle(filter_img, (int(4 * filter_img.shape[1] / 7), int(filter_img.shape[0] / 2)), int(filter_img.shape[1] / 4), 0, -1)
