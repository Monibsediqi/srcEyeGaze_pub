...
# --> USE VPYTHON TO CONSTRUCT A 3D ANDROID TAB A DEVICE AND VISUALIZE GAZE TRACKING VECTOR ON IT
# VPYTHON : GAZETRACKER
# x, y, z : x, -y, z
...
#                |     |                  |
scene_params = [(70, vec(100, 100, 180), vec(-0.8, -0.2, -0.6)),
                (150, vec(0, 75, 100), vec(0.0, -0.1, -0.6)),
                (100, vec(-100, 100, 180), vec(0.8, -0.2, -0.6)),
                (200, vec(0, 0, 000), vec(0.0, 0.0, 1.0))]
...
class Visualizer3d:
    ...
    def initialize_scene(self):
        ...
        scene.lights, scene.ambient = [], vec(1, 1, 1)
    ...
    def draw_axes(self):
        ...
        label(pos=vec(14, -0, 0), text='x')
        label(pos=vec(0, -14, 0), text='y')
        label(pos=vec(0, -0, 14), text='z')

    def draw_device_screen(self, device):

        # Draw and label device screen
        screen = box(pos=vec(0, screen_h_mm / 2 + 16, 0),
                     size=vec(screen_w_mm, screen_h_mm, 0),
                     color=vec(0.1, 0.1, 0.1))
        label(pos=vec(screen_w_mm / 2, screen.pos.y + screen_h_mm / 4, screen.pos.z),
              text='screen', xoffset=32, yoffset=32, linecolor=vec(0.5, 0.5, 0.5))

        # Draw device body: hardcoded to Nexus 7
        nexus_rect = shapes.rectangle(width=110, height=200, roundness=0.1)
        extrusion(path=[vec(0, 180 / 2, -2), vec(0, 180 / 2, -3)], shape=nexus_rect, color=vec(0.2, 0.2, 0.2))
        ...
        for i in range(4):
            for j in range(3):
                self.markers.append(
                    cylinder(pos=(j * 31 - 31, i * 35 + 40, 0), axis=(0, 0, 1), color=(1, 1, 1), radius=1))
