...
import find_eye_center_gradient
import find_eye_center_isophote
import image_utils
import draw_utils
...
def find_pupil(eye_img_bgr, fast_width_grads=25.5, fast_width_iso=80, weight_grads=0.9, weight_iso=0.1,
               debug_index=False):
    ...
    c_map_grads = eye_center_locator_gradient.get_center_map(fast_img_grads)
    c_map_iso = eye_center_locator_isophote.get_center_map(fast_img_iso)

    c_map_norm_grads = cv2.normalize(c_map_grads, 0, 255, norm_type=cv2.NORM_MINMAX)
    c_map_big_grads = cv2.resize(c_map_norm_grads, (eye_img_bgr.shape[1], eye_img_bgr.shape[0])).astype(np.uint8)

    c_map_norm_iso = cv2.normalize(c_map_iso, 0, 255, norm_type=cv2.NORM_MINMAX)
    ...
    if debug_index:
        ...
        draw_utils.draw_cross(debug_img, pupil_x0, pupil_y0, (0, 255, 255), 4, 1)
        draw_utils.draw_cross(joint_c_map, pupil_x0_3, pupil_y0_3, (255, 0, 255), 4, 1)
        draw_utils.draw_cross(joint_c_map, pupil_x0_2, pupil_y0_2, (255, 0, 255), 4, 1)
        draw_utils.draw_cross(joint_c_map, pupil_x0, pupil_y0, (255, 0, 0), 4, 2)

        draw_utils.draw_cross(c_map_big_iso, pupil_x0_3, pupil_y0_3, (255, 0, 0), 4, 1)
        draw_utils.draw_cross(c_map_big_grads, pupil_x0_2, pupil_y0_2, (255, 0, 0), 4, 1)
