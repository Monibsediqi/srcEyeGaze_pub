...
def get_obj_img_pts(board_img, debug=False):
...
def get_intrinsic_params(object_pts, image_pts, debug=False):
    _, cam_mat, dist_coefs, _, _ = cv2.calibrateCamera(object_pts, image_pts,img_size, None, None)
...
if __name__ == '__main__':
    ...
    calibration_imgs_path = 'chess_images'
    ...
    for img in calibration_imgs:
        object_pts, image_pts = get_obj_img_pts(img, debug=False)
        all_object_pts.append(object_pts)
        all_img_pts.append(image_pts)

    cam_mat, dist_coefs = get_intrinsic_params(all_object_pts, all_img_pts, True)

    params_file = open('cameras_matrix.txt', 'w')
    params_file.write(
