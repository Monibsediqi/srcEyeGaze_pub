...
def get_limb_pts(eye_img, phi=20, angle_step=1, debug_index=False):
    ...
    phi_range_1 = (
        (90 - phi) / angle_step, (90 + phi) / angle_step)  # Ranges of angles to be ignored (too close to lids)
    ...
    img_polar = linpolar(img_fixed_size, trans_w=int(polar_img_w), trans_h=int(__fixed_width / 2))
    ...
    filter_img.T[int(phi_range_1[0]): int(phi_range_1[1])] = 0
    filter_img.T[int(phi_range_2[0]): int(phi_range_2[1])] = 0
    ...
    if debug_index:
        ...
        cv2.circle(debug_img, (int(debug_img.shape[1] / 2), int(debug_img.shape[0] / 2)),
        ...
        if debug_index == 2:
            full_debug_img = stack_imgs_vertical([debug_img, eye_img_grey, stacked_imgs_polar]);
...
if __name__ == '__main__':
    ...
    tic = time()
    for a in range(100):
        pts_cart = get_limb_pts(eye_img, debug_index=2)
