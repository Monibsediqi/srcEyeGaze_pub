...
from conic_algorithm import Ellipse, BadEllipseShape
...
# print_point_x = 344.629
# print_point_y = 626.738
...
def get_err_scale(ellipse):
    ...
    pt1, pt2= x0 + (maj_axis / 2 + 1) * min_axis_x, y0 + (maj_axis / 2 + 1) * min_axis_y
    err_of_1px = ellipse.distance(pt1, pt2)
...
def ransac_ellipse_fit(points, bgr_img, roi_pos, ransac_iters_max=50, refine_iters_max=3, max_err=2, debug=False):
    ...
    for ransac_iter in range(ransac_iters_max):
        ...
        try:
            ...
            sample = random.sample(list(points_r), 3) + random.sample(list(points_l), 3)
            h, w = bgr_img.shape[:2]
            ellipse = fit_ellipse(sample, h, w)
            ...
            for (p_x, p_y) in sample:
                ...
                dx, dy = image_dx[int(p_y)][int(p_x)], image_dy[int(p_y)][int(p_x)]
                ...
                for _ in range(refine_iters_max):
                    ...
                    ellipse = fit_ellipse(inliers, h, w)
